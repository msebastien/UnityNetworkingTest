...
public class NetworkObjectPool : Singleton<NetworkObjectPool>
{
    ...
    [SerializeField]
    private List<PoolConfigObject> pooledPrefabsList = new List<PoolConfigObject>(); // Config for each prefab

    private HashSet<GameObject> prefabs = new HashSet<GameObject>(); // prefabs registered to the pool

    private Dictionary<GameObject, Queue<NetworkObject>> pooledObjects = new Dictionary<GameObject, Queue<NetworkObject>>(); // prefabs instances
    ...
    /// <summary>
    /// Gets an instance of the given prefab. The prefab must be registered to the pool.
    /// </summary>
